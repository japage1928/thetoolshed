...
async function portalConfiguration(stripe: ReturnType<typeof getStripeClient>['stripe']) {
  ...
  const active = existing.data.find((configuration) => (
    ...
    && configuration.business_profile.privacy_policy_url === `${siteUrl}/privacy-policy`
    && configuration.business_profile.terms_of_service_url === `${siteUrl}/terms`
  ...
  const configuration = await stripe.billingPortal.configurations.create({
    business_profile: {
      headline: 'Manage your Tool Shed Video Studio subscription',
