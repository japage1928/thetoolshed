...
async function runReport(accessToken: string, propertyId: string, body: Record<string, unknown>) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`GA4 report failed: ${response.status} ${detail}`);
  }
...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    if (!accessToken || !supabaseUrl || !supabaseKey) return json({ error: 'Unauthorized' }, 401);
    ...
    const overview = await runReport(googleToken, propertyId, {
      dateRanges,
      metrics: [
        { name: 'activeUsers' },
        { name: 'totalUsers' },
    ...
    const pages = await runReport(googleToken, propertyId, {
      dateRanges,
    ...
    const channels = await runReport(googleToken, propertyId, {
      dateRanges,
    ...
    return json({
      configured: true,
      measurementId: 'G-3CCH0VWGCF',
      range: 'Last 28 days',
      overview: { activeUsers: metricValues[0] || 0, totalUsers: metricValues[1] || 0, sessions: metricValues[2] || 0, pageViews: metricValues[3] || 0, events: metricValues[4] || 0 },
      pages: (pages.rows || []).map((row: any) => ({ path: row.dimensionValues?.[0]?.value || '/', views: Number(row.metricValues?.[0]?.value || 0), users: Number(row.metricValues?.[1]?.value || 0) })),
      channels: (channels.rows || []).map((row: any) => ({ channel: row.dimensionValues?.[0]?.value || 'Unknown', sessions: Number(row.metricValues?.[0]?.value || 0), users: Number(row.metricValues?.[1]?.value || 0) })),
